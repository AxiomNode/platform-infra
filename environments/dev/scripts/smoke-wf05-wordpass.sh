...
WORDPASS_BASE_URL="${WORDPASS_BASE_URL:-http://localhost:7101}"
AI_ENGINE_API_BASE_URL="${AI_ENGINE_API_BASE_URL:-http://localhost:7001}"
WF05_CATEGORY_ID="${WF05_CATEGORY_ID:-26}"
WF05_DIFFICULTY_PERCENTAGE="${WF05_DIFFICULTY_PERCENTAGE:-20}"
WF05_SINGLE_ITEM_COUNT="${WF05_SINGLE_ITEM_COUNT:-1}"
WF05_PROCESS_COUNT="${WF05_PROCESS_COUNT:-2}"
WF05_GENERATE_MAX_ATTEMPTS="${WF05_GENERATE_MAX_ATTEMPTS:-8}"
WF05_PROCESS_MAX_ATTEMPTS="${WF05_PROCESS_MAX_ATTEMPTS:-4}"
...
check_health() {
  ...
  if [[ "$status" != "200" ]]; then
    ...
    echo "  API_PORT=7001 docker compose -f ai-engine/src/docker-compose.yml --profile cpu up -d --build ai-cache llama-server-cpu ai-stats ai-api"
    echo "  docker compose -f microservice-wordpass/docker-compose.yml stop microservice-wordpass"
    echo "  docker compose -f microservice-wordpass/docker-compose.yml run -d --service-ports -e BATCH_GENERATION_ENABLED=false microservice-wordpass"
    exit 1
  fi
}
...
expect_process_created_with_retry() {
  local attempts="$1"
  local attempt=1

  while (( attempt <= attempts )); do
    process_response="$(request_with_status "POST" "$WORDPASS_BASE_URL/games/generate/process/wait" "{\"categoryId\":\"$WF05_CATEGORY_ID\",\"difficultyPercentage\":$WF05_DIFFICULTY_PERCENTAGE,\"count\":$WF05_PROCESS_COUNT}")"
    process_status="${process_response##*$'\n'}"
    process_payload="${process_response%$'\n'*}"

    if [[ "$process_status" != "201" ]]; then
      if [[ "$process_status" != "422" && "$process_status" != "502" && "$process_status" != "503" ]]; then
        echo "Unexpected status for POST $WORDPASS_BASE_URL/games/generate/process/wait: expected=201 got=$process_status"
        exit 1
      fi
    else
      ...
      echo "Retrying POST $WORDPASS_BASE_URL/games/generate/process/wait after created=0 (attempt $attempt/$attempts)" >&2
      ((attempt++))
      continue
    fi

    if (( attempt == attempts )); then
      echo "Unexpected status for POST $WORDPASS_BASE_URL/games/generate/process/wait after $attempts attempts: last_status=$process_status"
      exit 1
    fi

    echo "Retrying POST $WORDPASS_BASE_URL/games/generate/process/wait after transient status $process_status (attempt $attempt/$attempts)" >&2
    ((attempt++))
  done
}

check_health "ai-engine-api" "$AI_ENGINE_API_BASE_URL/health"
check_health "microservice-wordpass" "$WORDPASS_BASE_URL/health"

expect_status "400" "POST" "$WORDPASS_BASE_URL/games/generate" '{}'
expect_json_fields_with_retry "$WF05_GENERATE_MAX_ATTEMPTS" "POST" "$WORDPASS_BASE_URL/games/generate" "{\"categoryId\":\"$WF05_CATEGORY_ID\",\"difficultyPercentage\":$WF05_DIFFICULTY_PERCENTAGE,\"itemCount\":$WF05_SINGLE_ITEM_COUNT}" "gameType" "word-pass" "generated" "__NONEMPTY__"

history_before_response="$(request_with_status "GET" "$WORDPASS_BASE_URL/games/history?limit=1&page=1&pageSize=1&categoryId=$WF05_CATEGORY_ID&status=created")"
...
if [[ "$history_before_status" != "200" ]]; then
  echo "Unexpected status for GET $WORDPASS_BASE_URL/games/history before generation: expected=200 got=$history_before_status"
  exit 1
fi
history_before_total="$(get_json_field "$history_before_payload" "total")"

expect_process_created_with_retry "$WF05_PROCESS_MAX_ATTEMPTS"
...
history_after_response="$(request_with_status "GET" "$WORDPASS_BASE_URL/games/history?limit=1&page=1&pageSize=1&categoryId=$WF05_CATEGORY_ID&status=created")"
history_after_status="${history_after_response##*$'\n'}"
history_after_payload="${history_after_response%$'\n'*}"
if [[ "$history_after_status" != "200" ]]; then
  echo "Unexpected status for GET $WORDPASS_BASE_URL/games/history after generation: expected=200 got=$history_after_status"
  exit 1
fi
...
history_window_response="$(request_with_status "GET" "$WORDPASS_BASE_URL/games/history?limit=50&page=1&pageSize=50&categoryId=$WF05_CATEGORY_ID&status=created")"
...
if [[ "$history_window_status" != "200" ]]; then
  echo "Unexpected status for GET $WORDPASS_BASE_URL/games/history window scan: expected=200 got=$history_window_status"
  exit 1
fi
...
expect_random_matches_created_ids "$WORDPASS_BASE_URL" "word-pass" "$WF05_CATEGORY_ID" "$process_started_at" "$process_finished_at" "$created_ids_csv"

echo "WF-05 word-pass smoke OK (ai-engine -> word-pass generation -> persisted history growth -> exact random traceability)"